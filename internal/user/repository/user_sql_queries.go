...
const (
	...
	sqlGetUserByUsername = `SELECT id, user_id, username, first_name, last_name, email, is_active,created_at,updated_at, default_group_id
							FROM public.user
							WHERE username = $1`
	sqlGetUsersByGroupId = `SELECT id, user_id, username, first_name, last_name, email, is_active,created_at,updated_at, default_group_id from public.user u
							INNER JOIN public.usergroup_user ugu
							ON u.id = ugu.user_id
							WHERE ugu.usergroup_id = $1`
	sqlCreateUser        = `INSERT INTO public.user(user_id, username, first_name, last_name, email, is_active, created_at, updated_at, default_group_id) VALUES($1, $2, $3, $4, $5, $6, $7, $8)`
